...
class AsymmetricVerifier:
    """
    Holds a the asymmetric public key, used to verify signatures. It also
    contains methods to dump and load public keys.
    ...
    def public_to_b64(self) -> bytes:
        """
        Get the public key encoded as base64 format. Usefull when saving it
        using text format, or sending over the network.
        """
    ...
    def was_signed(public_key: ed25519.Ed25519PublicKey,
                   ...
                   data: bytes) -> bool:
        """
        Determine wheter the given signature is valid for the given data.
        The assigner's public key is used to verify such thing.
...
class AsymmetricKeyPair(AsymmetricVerifier):
    ...
    def from_pem_private_bytes(cls, private: bytes):
        ...
        privkey = serialization.load_pem_private_key(private,
                                                     None,
                                                     backend=default_backend())
        return cls(private=privkey)

    def private_to_pem(self) -> bytes:
        """
        Get the private key in the PEM encoding, using the PKCS8 format
        in plain text.
        """
        ...
        return self.__privkey.private_bytes(**kwargs)

    def sign(self, data: bytes) -> bytes:
        """
        Create a signature of the given data and return the signature
        as UTF-8 bytes.
